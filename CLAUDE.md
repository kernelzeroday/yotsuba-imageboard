...
- `localchan_config.php` — config engine (loads INI files, connects to DB)
...
- Apache serves from `/www/localchan/boards/` where each board is a directory of symlinks back to the PHP source
- Config uses a cascading INI system: global → category → board
- The `L::d()` helper in `lib/util.php` resolves board → domain mapping
- `YOTSUBA_DIR` config key points to `/www/global/localchan/` (symlink to `/var/www/html`)
...
- Cookie name is `localchan_pass`
- Database: `localchan` (MariaDB 10.1)
