...
function spam_filter_post_content_new($board, $resto, $com, $sub, $name, $upfile_name, $pwd = null, $pass_id = null) {
  ...
  $query = "SELECT id, pattern, autosage, log, regex, quiet, lenient, ops_only, min_count, board, ban_days FROM {$db->qi('postfilter')} WHERE active = 1 AND (board = '' OR board = ?)";
  ...
  if ($userpwd && $userpwd->isUserKnownOrVerified(1440) && $userpwd->postCount() > 10) { // 24 hours, 10 posts
    $user_is_known = true;
  }
  else {
    $user_is_known = false;
  ...
  while ($filter = $res->fetch(PDO::FETCH_ASSOC)) {
    ...
    // Lenient filter
    if ($user_is_known && $filter['lenient']) {
       continue;
    }
