...
function report_create_ban_req($board, $post_id, $template_id, $is_warn = false) {
  $post_id = (int)$post_id;
  $template_id = (int)$template_id;
  
  if ($template_id < 1 || $post_id < 1 || !$board) {
    return -1;
  }
  ...
  // Get the post
  $sql = "SELECT * FROM `%s` WHERE no = $post_id";
  ...
  // Get the template
  $sql = "SELECT * FROM ban_templates WHERE no = $template_id LIMIT 1";
  $res = mysql_global_call($sql);
  $tpl = mysql_fetch_assoc($res);
  
  if (!$tpl) {
    return -3;
  }
  
  $reason = $tpl['publicreason'];
  ...
  // Preserve the file
  if ($post['ext'] != '') {
    $img_filepath = IMG_DIR . "{$post['tim']}{$post['ext']}";
    $thumb_filepath = THUMB_DIR . "{$post['tim']}s.jpg";
    
    // Skip files for illegal reports
    $sql = "SELECT COUNT(*) FROM reports WHERE board = '%s' AND no = $post_id AND cat = 2";
    $res = mysql_global_call($sql, $board);
    $is_illegal = mysql_fetch_row($res)[0] > 0;
    
    if (!$is_illegal && $tpl['save_post'] === 'everything') {
  ...
  if ($post['resto']) {
    $sql = "SELECT sub FROM `%s` WHERE no = %d";
    $res = mysql_board_call($sql, $board, $post['resto']);
    $_sub = mysql_fetch_assoc($res);
    if ($_sub) {
      $rel_sub = $_sub['sub'];
  ...
  $tpl_name = $tpl['name'];
  $tpl_global = $tpl['bantype'] !== 'local' ? 1 : 0;
  
  $sql = <<<SQL
INSERT INTO ban_requests
SET host='%s', reverse='%s', pwd='%s', xff='%s', reason='', global = $tpl_global, tpl_name = '%s',
ban_template='%s', board='%s', janitor='%s', spost='%s', post_json='%s', warn_req = %d
SQL;
  
  $post['board'] = $board;
  $post_json = json_encode(generate_post_json($post, $post['resto'] ? $post['resto'] : $post['no'], [], true));
  
  $res = mysql_global_call($sql,
    $post['host'], $reverse, $post['pwd'], $xff, $tpl_name,
    $template_id, $board, 'Auto-ban', serialize($post), $post_json, $warn_req
  );
...
// FIXME:
// - cookie hackery to set the user name
// - the function can die and show an error
function report_delete_post($post_id) {
  $post_id = (int)$post_id;
  
  if (isset($_COOKIE['4chan_auser'])) {
    $_old_auser = $_COOKIE['4chan_auser'];
  }
  else {
    $_old_auser = false;
  }
  
  $_COOKIE['4chan_auser'] = 'Auto-ban';
  
  // post_id, pwd, $imgonly, automatic, children, die, lazy_rebuild, archived_deletion, tool
  delete_post($post_id, '', 0, 2, 1, 0, false, false, 'ban-req');
  
  if ($_old_auser !== false) {
    $_COOKIE['4chan_auser'] = $_old_auser;
  }
  else {
    unset($_COOKIE['4chan_auser']);
  }
}
...
function report_submit($board, $no, $cat_id) {
  ...
  if ($ignore_reason > 0) {
    ...
    if ($ignore_reason == 2) {
      ...
      log_spam_filter_trigger('ignore_report_score', $board, $no, $_SERVER['REMOTE_ADDR'], $ignore_reason, $_bot_headers);
    }
  }
  ...
  /*
  if ($board === 'test' && $resto && !$is_archived) {
    if ($cat_id == 39) {
      $template_id = 6; // Global 5 - NWS on Worksafe Board
    }
    else if ($cat_id == 35) {
      $template_id = 226; // Global 3 - Loli/shota pornography
    }
    else {
      $template_id = 0;
    }
    
    if ($template_id && $userpwd && $userpwd->isUserKnown(1440)) {
      $_res = mysql_board_call("SELECT email FROM `$board` WHERE no = $no LIMIT 1");
      $_ua = mysql_fetch_row($_res)[0];
      $_userinfo = decode_user_meta($_ua);
      
      if ($_userinfo['is_new']) {
        $_ret = report_create_ban_req($board, $no, $template_id);
        
        if ($_ret < 0) {
          fancydie('Error: ' . $_ret);
        }
        
        report_delete_post($no);
