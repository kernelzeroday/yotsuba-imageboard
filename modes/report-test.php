...
function report_create_ban_req($board, $post_id, $template_id, $is_warn = false) {
  ...
  $res = $db_board->query("SELECT * FROM $tbl WHERE no = ?", [$post_id]);
  ...
  // Get the template
  $db = YotsubaDB::global();
  $res = $db->query("SELECT * FROM ban_templates WHERE no = ? LIMIT 1", [$template_id]);
  $tpl = $res->fetch(PDO::FETCH_ASSOC);
  ...
  if ($post['ext'] != '') {
    ...
    $res = $db->query(
      "SELECT COUNT(*) FROM reports WHERE board = ? AND no = ? AND cat = 2",
      [$board, $post_id]
    );
    $is_illegal = $res->fetch(PDO::FETCH_NUM)[0] > 0;
  ...
  if ($post['resto']) {
    $res = $db_board->query("SELECT sub FROM $tbl WHERE no = ?", [(int)$post['resto']]);
    $_sub = $res->fetch(PDO::FETCH_ASSOC);
    if ($_sub) {
      $rel_sub = $_sub['sub'];
  ...
  $res = $db->query(
    "INSERT INTO ban_requests SET host=?, reverse=?, pwd=?, xff=?, reason='', global = ?, tpl_name = ?, ban_template=?, board=?, janitor=?, spost=?, post_json=?, warn_req = ?",
    [
      $post['host'], $reverse, $post['pwd'], $xff, $tpl_global, $tpl_name,
      (string)$template_id, $board, 'Auto-ban', serialize($post), $post_json, $warn_req
    ]
  );
...
function report_submit($board, $no, $cat_id) {
  ...
  if ($board === 'test' && $resto && !$is_archived) {
    ...
    if ($template_id && $userpwd && $userpwd->isUserKnown(1440)) {
      $db_board = YotsubaDB::board();
      $tbl = $db_board->qi($board);
      $_res = $db_board->query("SELECT email FROM $tbl WHERE no = ? LIMIT 1", [$no]);
      $_ua = $_res->fetch(PDO::FETCH_NUM)[0];
